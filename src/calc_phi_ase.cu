...
float calcPhiAse ( const ExperimentParameters& experiment,
     ...
     * Parse mesh
     ****************************************************************************/
    const Mesh<Acc,  DevAcc, StreamAcc>   dMesh(parseMesh<Acc,  DevAcc, StreamAcc> (compute.inputPath, devAcc));
    ...
    auto importanceWorkdiv = alpaka::workdiv::WorkDivMembers<Dim, Size>(importanceGrid, blocks);
    auto workdiv = alpaka::workdiv::WorkDivMembers<Dim, Size>(grid, blocks);
